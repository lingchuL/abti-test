...
export function ThemeToggle({ className = '' }: { className?: string }) {
  ...
  return (
    <button
      onClick={toggleTheme}
      className={`p-2 rounded-lg bg-white/80 dark:bg-slate-800/80 backdrop-blur border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-700 transition ${className}`}
    >
...
export function ThemeToggleSmall({ className = '' }: { className?: string }) {
  const { toggleTheme } = useTheme();

  return (
    <button
      onClick={toggleTheme}
      className={`p-1.5 rounded-lg hover:bg-white/50 dark:hover:bg-slate-800/50 transition ${className}`}
    >
